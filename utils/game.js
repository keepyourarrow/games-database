export const getRatingScore = (game) => {
  if (!game.rating) {
    return "Нету рейтинга";
  }

  return `${game.rating}/5`;
};

export const getRatingName = (ratings) => {
  if (!ratings || ratings.length == 0) {
    return "Нету рейтинга";
  }

  const maxPercent = ratings.reduce((p, c) => (p.percent > c.percent ? p : c));

  return maxPercent?.title;
};

export const getRatingsSorted = (ratings) => {
  return ratings.sort((a, b) => b.id - a.id);
};

export const typeHasTags = (type) => {
  const types = ["developers", "platforms", "publishers", "stores", "genres"];

  return types.find((item) => item == type);
};

export const getPlatformImage = (platform) => {
  const hash = {
    PC: "",
    XBOX: "",
    PLAYSTATION:
      "",
    NINTENDO:
      "",
    IOS: "",
    ANDROID:
      "iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAAAXNSR0IArs4c6QAAAC1JREFUWEft0EERAAAAAUH6lxbDZxU4s815PffjAAECBAgQIECAAAECBAgQIDAaPwAh6O5R/QAAAABJRU5ErkJggg==",
    MAC: "",
  };

  if (hash[platform.toUpperCase()]) {
    return hash[platform.toUpperCase()];
  } else {
    return "";
  }
};
