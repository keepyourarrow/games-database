              ...
              {searchResults.map((item) => (
                <SuggestionsItem key={item.id}>
                  <Link href={`/games/${item.slug}`}>
                    <SuggestionsImage img={item.background_image} onClick={handleClose}>
                      <span role="button" />
                    </SuggestionsImage>
                  </Link>
                  ...
                  <div>
                    <SuggestionsInfoPlatforms>
                      {item.parent_platforms?.slice(0, 3).map(({ platform }, index) => (
                        <PlatformIcon
                          key={index}
                          small={true}
                          style={{
                            backgroundImage: `url(
																data:image/svg+xml;base64,${getPlatformImage(platform.name)}
														)`,
                          }}
                        />
                      ))}
